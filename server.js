// ============================================================================
// Touch World - Socket Server v9.4.2 - Fixed Presence Sync
// ============================================================================
...
function normalizeUserShape(userAny) {
  ...
  const playerId = pd?.id ?? pd?.playerId ?? userAny?.id;
...
io.on("connection", async (socket) => {
  ...
  for (const [sid, p] of players.entries()) {
    if (p.playerId === user.playerId && sid !== socket.id) {
      console.log(`⚠️ Kicking duplicate: ${p.username}`);
  ...
  // ✅ שליחת snapshot מלא של כל השחקנים באזור
  ...
  console.log(`🟢 Connected: ${player.username} (${player.current_area}) | Peers: ${areaPeers.length}`);
...
// ✅ Presence Sync Loop - מונע היעלמות שחקנים
setInterval(() => {
  const areaGroups = new Map();
  
  // קיבוץ שחקנים לפי אזור
  for (const p of players.values()) {
    if (!areaGroups.has(p.current_area)) {
      areaGroups.set(p.current_area, []);
    }
    areaGroups.get(p.current_area).push(safePlayerView(p));
  }
  
  // שליחת snapshot מלא לכל אזור
  for (const [area, areaPlayers] of areaGroups) {
    io.to(area).emit("presence_sync", areaPlayers);
  }
}, 2000); // כל 2 שניות - מונע היעלמות
...
httpServer.listen(PORT, () => {
  ...
  console.log(`✅ Presence Sync: Every 2s (anti-disappear)`);
