...
async function checkScheduledAreas() {
  try {
    // Use AreaVersion instead of old Area entity
    const response = await fetch(`${BASE44_API_URL}/entities/AreaVersion`, {
      headers: {
        Authorization: `Bearer ${BASE44_SERVICE_KEY}`,
      },
    });
    ...
    for (const version of versions) {
      if (!version.scheduled_activation_date || version.is_active) continue;

      const scheduledDate = new Date(version.scheduled_activation_date);
      
      // אם הגיע הזמן
      if (scheduledDate <= now) {
        console.log(`⏰ Activating scheduled area version: ${version.area_name} (${version.version_name})`);

        // כבה את כל הגרסאות האחרות באותה תיקייה (folder_id)
        const sameFolder = versions.filter(v => v.folder_id === version.folder_id && v.id !== version.id);
        for (const other of sameFolder) {
          if (other.is_active) {
            await fetch(`${BASE44_API_URL}/entities/AreaVersion/${other.id}`, {
              method: 'PATCH',
              headers: {
                Authorization: `Bearer ${BASE44_SERVICE_KEY}`,
                'Content-Type': 'application/json',
        ...
        await fetch(`${BASE44_API_URL}/entities/AreaVersion/${version.id}`, {
          method: 'PATCH',
          headers: {
            Authorization: `Bearer ${BASE44_SERVICE_KEY}`,
            'Content-Type': 'application/json',
        ...
        console.log(`✅ Area version activated: ${version.area_name}`);
      }
    }
  } catch (error) {
